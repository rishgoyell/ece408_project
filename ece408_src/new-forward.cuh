...
// Compute C = A * B
__global__ void matrixMultiplyConstant(const float * __restrict__ Y, float * __restrict__ Z,
                                     const int B, const int M, const int C, const int H, const int W)
{
  ...
  const int numBColumns = OUTDIM1 * OUTDIM1 * B;
  ...
  int cs = OUTDIM1 * OUTDIM1;
  //int ks = K * K;
  int srow = (Col % (cs)) / OUTDIM1;
  int scol = (Col) % OUTDIM1; //(Col % (cs)) % W_out;
  int batch = blockIdx.z;

  #define Y4d(i3, i2, i1, i0) Y[(i3) * (C * H * W) + (i2) * (H * W) + (i1) * (W) + i0]
  #define Z4d(i3, i2, i1, i0) Z[(i3) * (M * OUTDIM1 * OUTDIM1) + (i2) * (OUTDIM1 * OUTDIM1) + (i1) * (OUTDIM1) + i0]
...
__global__ void matrixMultiplyRegister(const float * __restrict__ X,const float * __restrict__ Y, float * __restrict__ Z,
                                     const int B, const int M, const int C, const int H, const int W)

{
  const int numAColumns = ks * C;
  const int numBColumns = OUTDIM2 * OUTDIM2 * B;
  ...
  int cs = OUTDIM2 * OUTDIM2;
  //int ks = K * K;
  int srow = (N_col_to_load % (cs)) / OUTDIM2;
  int scol = (N_col_to_load % (cs)) % OUTDIM2;
  int batch = blockIdx.z;
  ...
  #define Y4d(i3, i2, i1, i0) Y[(i3) * (C * H * W) + (i2) * (H * W) + (i1) * (W) + i0]
  #define Z4d(i3, i2, i1, i0) Z[(i3) * (M * OUTDIM2 * OUTDIM2) + (i2) * (OUTDIM2 * OUTDIM2) + (i1) * (OUTDIM2) + i0]
  ...
  for(int it = 0; it < my_ceil(numAColumns,RATIO); it++)
  {
    ...
    for(int step = 0; step < RATIO; step++) {
      if (Row<M && it*RATIO + step < numAColumns)
  ...
  for(int reg = 0; reg < TILE_WIDTH_N; reg++)
  {
    if(Row < M && Col + reg < numBColumns)
    {
      int temp_col = Col + reg;
      int srow = (temp_col % (cs)) / OUTDIM2;
      int scol = (temp_col % (cs)) % OUTDIM2;
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
  ...
  //  const int K = w.shape_[3];
	...
	if(M==24) {
    	...
    	matrixMultiplyRegister<<<dimGrid, dimBlock>>>(w.dptr_, x.dptr_, y.dptr_ , B, M, C, H, W);
	}
    	...
    	matrixMultiplyConstant<<<dimGrid, dimBlock>>>(x.dptr_, y.dptr_ , B, M, C, H, W);
	}
